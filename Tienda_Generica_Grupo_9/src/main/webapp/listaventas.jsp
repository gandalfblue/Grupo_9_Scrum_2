...
<body>
	...
	<div id="layoutSidenav_content">
		...
		<div class="col-xl-8 col-lg-8 col-md-8 col-sm-8">
			...
			id="consecutivo">
				...
				<div class="row ">
					<div class="col-xl-2 col-lg-9 col-md-7 col-sm-6">
						<div class="input-group input-group-sm mb-3">
						<span class="input-group-text"> <i id="checkclientes"
	...
	function traerNombreCliente() {
		...
		req.open('GET', 'http://localhost:8080/consultarclientes?cedula_cliente=' +
			client, false);
		req.send(null);
		var cliente = null;
		if (req.status == 200)
		...
		console.log(cliente);
		var icono = document.getElementById("checkclientes");
		if (cliente.toString() != "") {
	...
	function traerNombreUsuario() {
		...
		var req = new XMLHttpRequest();
		var coincidencia = false;
		req.open('GET', 'http://localhost:8080/consultarusuario_cedula?cedula_usuario=' +
			user, false);
		req.send(null);
		var usuario = null;
		if (req.status == 200)
	...
	function cargarProducto1() {
		...
		var req = new XMLHttpRequest();
		var coincidencia = false;
		req.open('GET', 'http://localhost:8080/consultarproductos?codigo_producto=' +
			prod1, false);
		req.send(null);
		var producto1 = null;
		if (req.status == 200)
	...
	function cargarProducto3() {
		...
		var req = new XMLHttpRequest();
		var coincidencia = false;
		req.open('GET', 'http://localhost:8080/consultarproductos?code=' +
			prod3, false);
		req.send(null);
		var producto3 = null;
		if (req.status == 200)
	...
	function calcularPrecio2() {
		...
		if (cantidad == "" || cantidad == null) {
			...
			valor_total = parseFloat(cantidad) * precio1;
			console.log("Valor 2: " + valortotal);
			document.getElementById("valtotal2").value = valor_total;
		}
		calcularFinales();
	}
	function calcularPrecio3() {
		...
		if (cantidad == "" || cantidad == null) {
			...
			document.getElementById("valtotal3").value = valor_total;
		}
		calcularFinales();
	}
	function calcularFinales() {
		...
		document.getElementById("total_venta_iva").value = iva + total_venta;
	}
	function registrar() {
		try {
			...
			xhr.open("POST", "http://localhost:8080/registrarventas");
			xhr.send(formData);
			setTimeout(
				function() {
					for (i = 1; i < 4; i++) {
